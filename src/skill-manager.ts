...
export default function skillManager(pi: ExtensionAPI) {
  ...
  function clearReloadStatus(ctx: any): void {
    ctx.ui?.setStatus?.(STATUS_ID, undefined);
  }

  function notifyMutation(ctx: any, message: string, type: 'info' | 'success' | 'error' = 'info'): void {
  ...
  function startWatcherIfEnabled(ctx: any): void {
    ...
    if (process.env.SKILL_MANAGER_WATCH !== '1') return;
    ...
    async execute(_toolCallId, params: any, _signal, _onUpdate, ctx: any) {
      ...
      switch (action) {
        ...
        case 'import': {
          ...
          if (result!.success) notifyMutation(ctx, `Imported skill "${name}"`, 'success');
          return {
        ...
        case 'activate': {
          ...
          if (actResult.success) notifyMutation(ctx, `Activated skill "${name}" (${scope})`, 'info');
          return {
        ...
        case 'deactivate': {
          ...
          if (deactResult.success) notifyMutation(ctx, `Deactivated skill "${name}" (${scope})`, 'info');
          return {
        ...
        case 'list': {
          ...
          ctx.ui?.notify?.(
            `📋 ${vaultSkills.length} skills in vault(s), ${userActive.length} active (user), ${projectActive.length} active (project)`,
            'info',
          );
          return {
        ...
        case 'remove': {
          ...
          if (rmResult.success) notifyMutation(ctx, `Removed skill "${name}"`, 'info');
          return {
        ...
        case 'update': {
          ...
          if (result!.success) notifyMutation(ctx, `Updated skill "${name}"`, 'success');
          return {
            content: [{ type: 'text', text: formatMessage(result!) }],
            details: { result },
          };
